...
**2026-09-18 게시 전 최종 점검 결과.** 심사 통과한 zip으로 실제 Chromium 테스트를 돌렸습니다. 기존 smoke 11개 통과, 새 Pro 테스트(`test/pro.mjs`, 라이선스 서버는 모의 응답) 35개 중 29개 통과. 실패 6개는 아래 5-4의 버그 세 개(동기화, ModHeader 가져오기, 열어 둔 옵션 탭)를 가리킵니다. 결제·활성화·환불 감지·오프라인 유예·기기 해제·정규식·프로필·내보내기는 정상입니다.

게시 순서 (권장):
1. **실제 결제 테스트 (사람).** Lemon Squeezy에서 100% 할인 코드를 만들어 $0로 직접 사 보고, 메일로 온 키를 확장에 넣어 "Pro active"가 뜨는지 확인합니다. `config.js` 의 productId(1362488)가 진짜 상품 ID인지 확인할 방법은 이것뿐입니다. 틀렸으면 모든 구매자가 "This key belongs to a different product" 오류를 봅니다
2. **Lemon Squeezy 상품 설명에서 sync 빼기 (사람).** 동기화가 고쳐질 때까지 팔지 않습니다. 사이트와 LAUNCH.md에서는 이미 뺐습니다
3. 게시 버튼 누르기. 단, **홍보(Show HN, Reddit, Product Hunt)는 1.0.1 승인 뒤에** 합니다. 사람이 몰렸을 때 버그를 만나면 첫인상이 굳습니다
...
아래 항목을 한 번에 올립니다. 고친 뒤 `PW_EXPERIMENTAL_SERVICE_WORKER_NETWORK_EVENTS=1 node test/pro.mjs` 가 전부 통과해야 합니다.

**동기화가 사실상 동작하지 않음 (버그, 1순위).** `saveState` 가 로컬 상태를 `chrome.storage.sync` 에 덮어쓰기만 하고, 읽어 오는 곳은 옵션 페이지를 처음 열 때 한 번뿐입니다. 두 번째 기기에서 Pro를 켜고 동기화를 체크하는 순간 그 기기의 기본 프로필이 클라우드 사본을 덮어써서, 첫 기기의 프로필은 영영 넘어오지 않습니다. 게다가 전체 프로필을 한 항목에 넣어서 항목당 8KB 한도를 넘으면(긴 토큰 10개 정도) 조용히 저장 실패합니다. 입력할 때마다 쓰기가 일어나 분당 120회 한도에도 걸릴 수 있습니다. 고칠 것: 동기화를 켤 때 클라우드에 데이터가 있으면 "이 기기로 가져오기 / 이 기기 것으로 덮어쓰기"를 묻기, `storage.onChanged`(sync)로 다른 기기 변경 받기, 여러 항목으로 나눠 저장, 쓰기 모으기(debounce). 고치기 전까지 사이트·LAUNCH·Lemon Squeezy 설명에서 sync를 뺀 상태로 둡니다

**열어 둔 옵션 탭이 팝업에서 고친 규칙을 되돌림 (버그, 데이터 손실).** 옵션 페이지는 열 때 상태를 한 번 읽고 그 복사본을 통째로 저장합니다. 설치하면 옵션 탭이 자동으로 열리는데, 그 탭을 둔 채 팝업에서 규칙을 만들고 나중에 그 탭에서 Pro 키를 활성화(또는 설정 변경)하면 그동안 만든 규칙이 사라집니다. 구매 직후에 규칙이 날아가는 최악의 타이밍입니다. 고칠 것: 옵션 페이지도 `storage.onChanged` 로 상태를 갱신하고, 저장할 때는 통째로 덮지 말고 바뀐 부분만 `updateState` 로 반영

**오프라인일 때 활성화 오류 문구.** 지금은 "Failed to fetch"가 그대로 보입니다. "라이선스 서버에 연결할 수 없습니다. 인터넷 연결을 확인하세요"에 해당하는 영어 문구로 바꿉니다
...
**ModHeader URL 필터와 프로필 이름도 가져오기 (중요).** 지금 가져오기는 헤더 이름·값·켜짐 상태·메모만 옮기고 URL 필터는 버립니다. 프로필 이름도 ModHeader가 쓰는 `title` 키를 읽지 않아 "Imported 1"이 됩니다(`name` 만 읽음). 실제 ModHeader 내보내기 파일 하나로 키 이름을 꼭 확인하세요. 그래서 스테이징 API에만 걸어 두던 Authorization 토큰 규칙이 가져온 뒤에는 모든 사이트로 나갑니다. 사이트 Switching 3단계에 "필터부터 다시 넣으라"고 경고를 넣어 두었지만, 코드로 막는 게 맞습니다. ModHeader 내보내기의 URL 필터를 Headrule 필터로 옮기고, 옮길 수 없는 규칙은 꺼진 상태로 가져오게 합니다. 고치면 사이트 Switching 3단계를 지웁니다.
